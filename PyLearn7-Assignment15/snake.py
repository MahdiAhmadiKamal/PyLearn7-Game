...
class Snake(arcade.Sprite):
    ...
    def move_ai(self, apple, pear):
        self.body.append({'x': self.center_x, 'y': self.center_y})

        if len(self.body)> self.score:
            self.body.pop(0)

        apple_distance = sqrt((self.center_x - apple.center_x)**2 + (self.center_y - apple.center_y)**2)
        pear_distance = sqrt((self.center_x - pear.center_x)**2 + (self.center_y - pear.center_y)**2)

        if pear_distance <= apple_distance:
            if self.center_x < pear.center_x:
                self.center_x += 1 * self.speed
            if self.center_y < pear.center_y:
